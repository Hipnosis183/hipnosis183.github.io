...
For the graphical department, a video wrapper will be included (the magnificent **dgVoodoo**), to max the resolution to desktop. Additionally, a widescreen patch will be available, which envolves some nice math. All of this, implemented in a dynamic way, so no manual configuration should be involved, other than activating or deactivating the features.
...
It wasn’t a very stupid idea after all. As Process Monitor shows, the file is beign correctly read from the game directory, and now everything is working as expected. Also, the same applies to te setup program, which shares the same code. Patching this file is also important.
...
We first calculate the horizontal FOV, and with that, the vertical FOV. This give us a decimal value, which then we convert to hex. To understand it more easily, take it as a *cross-multiplication*: if `1.33` (4/3) is `1` (FOV), then what FOV will `1.78` (16/9) have? Obviously this is oversimplified, and is not used because one formula may not work in all the possible cases.
